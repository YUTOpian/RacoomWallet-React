...
import {
  fetchNemBalance, estimateNemSendFee, sendNemTransfer,
} from '../../lib/nemChain';
import { isValidNemAddress, extractNemAddressFromQr } from '../../lib/nemQr';
import NemHero from '../../components/NemHero';
import { useAppStore } from '../../store/appStore';

// NEM's brand blue - the same swatch as the balance card gradient (NemTop.tsx) and
// NemHero, used here for the 全額 button and the toolbar's back arrow so this screen reads
// consistently with the rest of the NEM section rather than the app's default teal.
const NEM_BLUE = '#2F7FCC';

type Step = 'form' | 'confirm' | 'complete';

export default function NemSend() {
  const navigate = useNavigate();
  const [searchParams] = useSearchParams();
  // Set when this screen was reached via Home's SEND button -> the numpad screen (see
  // pages/top/Send.tsx and pages/send/SendNemAmount.tsx), rather than via the NEM tab's
  // own 送信 button. Changes two things: the recipient/amount fields below start
  // pre-filled from what was already entered on those screens, and completing the send
  // returns to the Home screen instead of the NEM screen (see the 'complete' step and
  // onPassed below).
  ...
  const onDecodeQr = (decoded: string) => {
    const address = extractNemAddressFromQr(decoded);
    if (!address) {
      setErrorMessage("Couldn't read a NEM address from the QR code");
      return;
    }
  ...
  const onConfirm = async () => {
    let hasError = false;
    const cleanedRecipient = recipient.trim().toUpperCase().replace(/-/g, '');
    if (!isValidNemAddress(cleanedRecipient)) {
      setAddressError('Please enter a valid NEM address (starting with N)');
      hasError = true;
    } else {
      setAddressError('');
    }
    ...
    setRecipient(cleanedRecipient);
    setStep('confirm');
    setEstimatedFee('loading');
    try {
      const fee = await estimateNemSendFee(cleanedRecipient, amount, message);
      setEstimatedFee(fee);
    } catch (e) {
      console.error('Failed to estimate NEM send fee', e);
  ...
  const onPassed = async (pin: string) => {
    ...
    try {
      ...
      const result = await sendNemTransfer(account.privateKeyHex, recipient.trim().toUpperCase(), amount, message);
      setTxHash(result.hash);
      setShowPinDialog(false);
      setStep('complete');
      if (fromHome) {
        // These were only staged here as a one-shot handoff from the Home send flow
        // (see SendNemAmount.tsx) - clear them now so they don't linger and get
        // reused if the person starts an unrelated send later.
        clearReceiverAddress();
        clearCalculatorFormula();
      }
    } catch (e) {
      console.error('Failed to send NEM transfer', e);
  ...
  if (step === 'complete') {
    return (
      <Box sx={{ height: '100vh' }}>
        <AppToolBar back="/nem" title="Send complete" showBack={false} />
        <Box sx={{ display: 'flex', flexDirection: 'column', alignItems: 'center', mt: 8, px: 3, gap: 2 }}>
          ...
          <Button
            variant="contained"
            disableElevation
            onClick={() => navigate(fromHome ? '/top?tab=home' : '/nem', { replace: true })}
          >
            {fromHome ? 'Back to home' : 'Back to NEM screen'}
          </Button>
        </Box>
      </Box>
    );
  ...
  if (step === 'confirm') {
    return (
      <Box sx={{ height: '100vh' }}>
        <AppToolBar back="/nem/send" title="Confirm transfer details" onBack={() => setStep('form')} backColor={NEM_BLUE} />
        <Box sx={{ display: 'flex', flexDirection: 'column' }}>
          ...
          <Box sx={{ px: 2 }}>
            ...
            <Typography sx={{ fontSize: 'large' }}>{amount} XEM</Typography>

            <Typography sx={{ color: NEM_BLUE, mt: 1 }}>Recipient address</Typography>
            <Typography sx={{ wordBreak: 'break-all' }}>{recipient.trim().toUpperCase()}</Typography>

            {message && (
              <>
                <Typography sx={{ color: NEM_BLUE, mt: 1 }}>Message</Typography>
                <Typography sx={{ wordBreak: 'break-all' }}>{message}</Typography>
              </>
            )}

            <Typography sx={{ color: NEM_BLUE, mt: 1 }}>Fee (estimated)</Typography>
            <Typography>
        ...
        <Box sx={{ position: 'sticky', bottom: 0, maxWidth: 480 }}>
          <AppBar position="static" color="default">
            <Toolbar sx={{ justifyContent: 'center' }}>
              ...
              <IconButton
                ...
                sx={{ bgcolor: NEM_BLUE, color: 'white', mx: 1, width: 40, height: 40, '&:hover': { bgcolor: NEM_BLUE } }}
              >
          ...
          <Box sx={{ width: '100%', height: 4, bgcolor: NEM_BLUE }} />
        </Box>
  ...
  return (
    <Box>
      <AppToolBar back={fromHome ? '/send/nem-amount' : '/nem'} title="Send" backColor={NEM_BLUE} />
      <Box sx={{ display: 'flex', flexDirection: 'column' }}>
        <WalletBar isOpened={false} />
        <NemHero />

        <Box sx={{ px: 2, display: 'flex', flexDirection: 'column', gap: 2 }}>
          <Typography sx={{ color: 'text.secondary', fontSize: 13 }}>残高: {balance} XEM</Typography>

          <TextField
            label="Recipient address"
            placeholder="40 characters starting with N"
            fullWidth
            value={recipient}
          ...
          <TextField
            ...
            slotProps={{ input: { endAdornment: <Button size="small" onClick={onSetMax} sx={{ color: NEM_BLUE }}>Max</Button> } }}
          />
          ...
          <Button
            ...
            size="large"
            onClick={onConfirm}
            sx={{ bgcolor: NEM_BLUE, '&:hover': { bgcolor: NEM_BLUE } }}
          >
            次へ
          </Button>
        </Box>
