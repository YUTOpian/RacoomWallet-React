...
import NemBackupHero from '../../components/NemBackupHero';
...
// NEM (XEM) private-key backup - separate from WalletBackup.tsx (EVM key/mnemonic) and
// from SymbolBackup.tsx (Symbol's ed25519 key), for the same reason SymbolBackup.tsx gives
// for itself: NEM uses its own ed25519 keypair, deterministically derived from the same
// EVM key but with a different salt (see lib/nemAccount.ts), so it isn't interchangeable
// with either of the other two and needs its own decrypt-then-derive step and screen.
export default function NemBackup() {
  ...
  const onUnlockPassed = async (pin: string) => {
    ...
    try {
      ...
      const account = NemAccountHelper.fromPrivateKey(evmPrivateKey);
      setPrivateKey(account.privateKeyHex);
      setLoadError(account.privateKeyHex.trim().length === 0);
      setState('ready');
      setShowPinDialog(false);
    } catch (e) {
      console.error('NemBackup: failed to derive NEM private key', e);
      setLoadError(true);
      setState('ready');
      setErrorMessage('Failed to generate the NEM private key');
    }
  };
  ...
  return (
    <Box sx={{ width: '100vw', pb: `${NEM_BOTTOM_NAV_HEIGHT}px` }}>
      <AppToolBar back="/nem" title="NEM Backup" />
      ...
      <NemBottomNav active="backup" onHarvestClick={() => navigate('/nem/harvest')} hideOther />
      ...
      <MessageDialog
        open={copied}
        title="Copied"
        texts={['Your NEM private key has been copied to the clipboard.']}
