...
export default function LessonLoginEnd() {
  const { t } = useTranslation();

  return (
    <Box sx={{ minHeight: '100vh' }}>
      <AppToolBar back="/lesson/login" title={t('lesson.login_end_title')} />
      <Box component="img" src={heroSeclessonLarge} sx={{ width: '100%' }} />
      <Box sx={{ mb: 5, mx: 2 }}>
        <Typography align="center">
          {(t('lesson.login_end_message', { returnObjects: true }) as string[]).map((line, i) => (
